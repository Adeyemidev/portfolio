...
const Header = () => {
  ...
  const {isLoaded} = usePageLoad(); // ✅ Keep this
  ...
  return ( 
    <section className="px-6 min-h-screen block items-center relative pt-20">
        
        <div className="mb-16 ml-10 mt-6">
          <span 
            style={{opacity:"0.5"}} 
      ...
      >
        ...
        <div id="app">
          <div id="wrapper">
            <h1 className="glitch text-center" data-text="I'M ADEYEMI EZEKIEL">
              I'M ADEYEMI EZEKIEL
            </h1>
          </div>
        </div>
        ...
        {headerData.paragraphs.map((paragraph, index) => (
          <p 
            key={index}
            style={{opacity: "0.9"}} 
            className="text-lg md:text-xl lg:text-2xl text-slate-300 my-6 max-w-4xl leading-relaxed mx-auto"
          >
            {paragraph}
          </p>
        ))}
        <div className="flex flex-col sm:flex-row mt-5 gap-4 justify-center items-center">
          <button className="group border bg-white px-8 py-4 rounded-full font-medium text-black transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-green-500/25"> 
